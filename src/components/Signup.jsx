...
const Signup = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.role === 'student' || formData.role === 'admin') {
      if (!formData.email) {
        setError(`${formData.role === 'student' ? 'Student' : 'Admin'} requires email`);
        return false;
      }
    }
    ...
    if (formData.role === 'driver') {
      if (!formData.phone) {
        setError('Driver requires phone number');
        return false;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare payload based on role (matching React Native logic)
    const payload =
      formData.role === 'student'
        ? { name: formData.name, email: formData.email, password: formData.password, role: formData.role }
        : formData.role === 'admin'
        ? { name: formData.name, email: formData.email, password: formData.password, role: formData.role }
        : { name: formData.name, phone: formData.phone, password: formData.password, role: formData.role };
